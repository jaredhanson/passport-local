 ...
 *.         **Default:** `false`.
 * @param {verifyFn|verifyWithReqFn} verify
 *
 * @example
 ...
 * Verifies `username` and `password` and yields authenticated user.
 *
 * This function is called by `{@link Strategy}` when the `passReqToCallback`
 * option is set to verify a username and password, and must invoke `cb` to
